...
skmnew$Meals_At_Home <- impute_with_mean(skmnew$Meals_At_Home)
skmnew$Meals_Employer <- impute_with_mean(skmnew$Meals_Employer)
skmnew$Meals_Payment <- impute_with_mean(skmnew$Meals_Payment)
...
for (col in outlier_columns) {
  skmnew <- remove_outliers(skmnew, col)
}

# Summarize consumption
skmnew$total_consumption <- rowSums(skmnew[, c("ricepds_v", "Wheatpds_q", "chicken_q", "pulsep_q", "wheatos_q")], na.rm = TRUE)
...
district_mapping <- c("4" = "East", "3" = "South", "2" = "West")
sector_mapping <- c("2" = "URBAN", "1" = "RURAL")

skmnew$District <- as.character(skmnew$District)
skmnew$Sector <- as.character(skmnew$Sector)
skmnew$District <- ifelse(skmnew$District %in% names(district_mapping), district_mapping[skmnew$District], skmnew$District)
skmnew$Sector <- ifelse(skmnew$Sector %in% names(sector_mapping), sector_mapping[skmnew$Sector], skmnew$Sector)
